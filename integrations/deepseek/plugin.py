...
def _key(cfg: Dict[str,Any])->str:
    return (cfg.get("api_key") or os.environ.get("DEEPSEEK_API_KEY") or "").strip()

def init_plugin(config: Dict[str, Any], registry: PluginRegistry, manifest: PluginManifest) -> None:
    model = str(config.get("model") or os.environ.get("DEEPSEEK_MODEL") or "deepseek-chat")
    base_url = str(config.get("base_url") or "https://api.deepseek.com/v1").rstrip("/")
    ...
    def chat(prompt: str, system: Optional[str]=None, temperature: Optional[float]=None)->Dict[str,Any]:
        ...
        if not prompt.strip(): raise ValueError("prompt required")
    ...
    registry.register_tool(ToolRegistration("deepseek.chat", manifest.id, "deepseek", "deepseek_chat", chat, "DeepSeek chat.completions (approval)", model))
